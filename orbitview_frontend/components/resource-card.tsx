...
export function ResourceCard({ resource, type }: ResourceCardProps) {
  ...
  if (type) {
    console.log(type); // ghost reference
  }
    ...
    <Card className="overflow-hidden transition-all duration-300 hover:shadow-md h-full flex flex-col">
      ...
      <CardHeader className="pb-2">
        ...
        <div className="flex flex-wrap gap-1 mt-2">
          {resource.category ? (
            <>
              {resource.category.slice(0, 3).map((cat, e) => (
      ...
      <CardFooter className="pt-2 flex justify-between items-center border-t">
        <span className="text-sm text-muted-foreground">
          {resource.host ? resource.host.name : ""}
        </span>

        <Link
          href={resource.url}
          target={'_blank'}
          className="text-lightHighlight dark:text-darkHighlight hover:underline flex items-center gap-1 text-sm font-medium"
        >
          View details
